...
pub fn handle(
    ...
    // We only need the multisig_state PDA account
    assert!(accounts.len() >= 1, "CreateMultisig requires multisig_state account");
    ...
    // For ZK-based private multisig, members[] stores NPKs only.
    // No member accounts needed — identity is proven via ZK proofs, not account ownership.
    ...
    // No member accounts to claim — ZK-based identity
