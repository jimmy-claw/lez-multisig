...
mod tests {
    ...
    #[test]
    fn test_remove_member_threshold_met() {
        ...
        let (post_states, _) = handle(&accounts, &[3u8; 32]); // Remove member 3
    ...
    fn test_remove_member_threshold_not_met() {
        let members = vec![[1u8; 32], [2u8; 32], [3u8; 32]];
    ...
    fn test_remove_member_not_found() {
        ...
        handle(&accounts, &[9u8; 32]);
    }

    #[test]
    #[should_panic(expected = "Threshold cannot exceed member count")]
    fn test_remove_member_threshold_exceeds() {
        let members = vec![[1u8; 32], [2u8; 32]];
        let state_data = make_multisig_state(2, members); // threshold = 2, members = 2
        ...
        // Remove member would leave 1 member but threshold is 2
        handle(&accounts, &[2u8; 32]);
    }
}
