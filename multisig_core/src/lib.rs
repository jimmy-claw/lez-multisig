...
//
// LP-0002: Voting is ZK-based — members prove membership via vote_circuit
// receipts. Only vote nullifiers appear on-chain; voter identity is private.
...
///    (membership proven via ZK proof, not signer check)
...
pub struct Proposal {
    ...
    pub index: u64,
    /// Who proposed it (account ID — [0u8; 32] for private proposals)
    pub proposer: [u8; 32],
    /// Nullifier committed by the proposer (derived via vote_circuit)
...
impl Proposal {
    ...
    /// Add an approval (by nullifier). Returns true if this was a new approval.
    pub fn approve(&mut self, nullifier: [u8; 32]) -> bool {
        if self.approved.contains(&nullifier) {
            return false;
        }
        self.rejected.retain(|r| r != &nullifier);
        self.approved.push(nullifier);
        true
    }

    /// Add a rejection (by nullifier). Returns true if this was a new rejection.
    pub fn reject(&mut self, nullifier: [u8; 32]) -> bool {
        if self.rejected.contains(&nullifier) {
            return false;
        }
        self.approved.retain(|a| a != &nullifier);
        self.rejected.push(nullifier);
        true
    }
...
pub struct MultisigState {
    ...
    /// Number of members (N)
    pub member_count: u8,
    /// List of member NPKs (nullifier public keys)
